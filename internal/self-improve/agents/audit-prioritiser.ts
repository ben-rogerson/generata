...
Procedure:
1. Locate the fenced JSON block (\`\`\`json ... \`\`\`) inside SCANNER OUTPUT and parse it. If no fenced block is present, attempt to parse the largest \`{...}\` substring. If parsing still fails, print a one-line error to stdout starting with \`ERROR:\` describing what was wrong, and exit non-zero.
...
3. Apply lens weighting: multiply base_score by 1.2 if lens is "dx-api" or "consistency"; otherwise 1.0. Round to the nearest integer. This honours the spec's "soft lean toward dx-api + consistency" rule.
4. Drop any finding whose evidence_paths reference an out-of-scope location:
   - .changeset/, CHANGELOG.md, package.json (any version field)
   - .github/workflows/
   - internal/self-improve/
5. Dedup near-duplicates: if two findings share a lens AND any evidence path, keep the higher-scored one.
6. Sort descending by score; ties broken by lower effort (i.e. shippability).
