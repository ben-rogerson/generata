...
export default defineAgent<{ slug: string; diff_filepath: string }>(
  ({ slug, diff_filepath, work_dir }) => {
...
Procedure:
1. Read ${diff_filepath} for the change content. Use \`git -C ${repo_root} diff --stat HEAD\` if you need the per-file line-count summary for the artifact below. **Do not invoke python, node, or other shell interpreters.** Do not consult prior commits (no \`git log\`, no \`HEAD^\`); the diff against working tree is the change.
