...
function renderSlashCommand(wf: WorkflowDef, pm: string): string {
  ...
  const optionalVars = variables.filter((v) => !requiredSet.has(v));
  const allVars = [...required, ...optionalVars];
  const argumentHint = allVars.length
    ? allVars.map((v) => `--${v} <value>`).join(" ")
    : "[--<var> <value>]";
  return `---
description: ${wf.description}
argument-hint: ${argumentHint}
---
<!-- Generated by generata from agents/${wf.name}.ts. -->
